...
class ventas_pedido extends fbase_controller
{
        ...
        if ($this->pedido->editable) {
            ...
            if (isset($_POST['numlineas'])) {
                ...
                foreach ($lineas as $l) {
                    ...
                    if (!$encontrada) {
                        // Quitar los reservados de esta línea
                        $reserva = new linea_pedido_cliente_reservado();
                        $reserva = $reserva->get($l->idlinea);
                        if(!$reserva || $reserva->eliminar_reserva()) {
                            if($l->delete()) {
                                continue;
                            }
                        }
            ...
            // Cambiamos las cantidades reservadas
            foreach($_POST as $name=>$var) {
                $linea_reserva = new linea_pedido_cliente_reservado();
                if(substr_compare($name,"reservado_",0,10) == 0) {
                    $idlinea = +substr($name,10);
                    $l = $linea_reserva->get($idlinea);
                    $nuevo_valor = +$var;
                    if(!$l) {
                        $l = new linea_pedido_cliente_reservado(array("idlinea"=>$idlinea,"stockreservado"=>0));
                    }
                    if($l->stockreservado != $nuevo_valor) {
                        $l->set_reservado($nuevo_valor);
                    }
                }
            }
        }
    ...
    private function generar_albaran()
    {
        ...
        if (!$eje0) {
            ...
            foreach ($this->pedido->get_lineas() as $l) {
                ...
                if ($n->save()) {
                    /// descontamos del stock y quitamos los reservados
                    if ($n->referencia) {
    ...
    public function get_lineas_stock()
    {
        ...
        new linea_pedido_cliente_reservado();

        $sql = "SELECT l.idlinea,l.referencia,l.descripcion,l.cantidad,s.disponible as stock,le.stockreservado as reservado,s.ubicacion FROM lineaspedidoscli l LEFT JOIN lineaspedidosclireservado le ON l.idlinea = le.idlinea"
        ...
        if ($data) {
            ...
            foreach ($data as $d) {
                ...
                if ($articulo) {
                    ...
                    if(!$articulo->nostock && $articulo->tipo != "atributos") {
                        $linea['reservado'] = isset($d['reservado']) ? $d['reservado'] : "0";
                    }
                    $lineas[] = $linea;
                }
            }
    ...
    private function comprobar_stock()
    {
        ...
        $art0 = new articulo();
        // Almacenamos los comprobados y sus cantidades por si el mismo articulo vuelve a 
        // aparecer, tener en cuenta que el anterior ya habría utilizado stock
        $anteriores = array();
        foreach ($this->pedido->get_lineas() as $linea) {
            if ($linea->referencia) {
                    ...
                    if ($linea->cantidad > $articulo->stockfis) {
                        ...
                        foreach ($articulo->get_stock() as $stock) {
                            if ($stock->codalmacen == $this->pedido->codalmacen) {
                                $reservado = new linea_pedido_cliente_reservado();
                                $reservado = $reservado->get($linea->idlinea);
                                $disponible = $stock->disponible;
                                if($reservado) {
                                    $disponible += $reservado->stockreservado;
                                }
                                if(isset($anteriores[$linea->referencia])) {
                                    $disponible -= $anteriores[$linea->referencia];
                                }
                                if ($disponible >= $linea->cantidad) {
                                    $anteriores[$linea->referencia] = $linea->cantidad + (isset($anteriores[$linea->referencia]) ? $anteriores[$linea->referencia] : 0);
                                    $ok = TRUE;
                                }
                                break;
                            }
