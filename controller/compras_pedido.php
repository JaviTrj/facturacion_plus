    ...
    private function generar_albaran()
    {
        ...
        if (!$eje0) {
            ...
            foreach ($this->pedido->get_lineas() as $l) {
                // Si el pedido fue confirmado, revisamos si la línea lo fue
                if(isset($_POST["comprobado"])) {
                    $confirmados = +$_POST["recibido_".$l->idlinea];
                    if($confirmados < $l->cantidad) {
                        $modificado = TRUE;
                        // Si se va a crear un pedido con los que faltan almacenarlos
                        if(isset($_POST["crear_pendientes"])) {
                            $pendiente = new pendiente_proveedor();
                            $pendiente->referencia = $l->referencia;
                            $pendiente->idalbaran = $albaran->idalbaran;
                            $pendiente->cantidad = $l->cantidad - $confirmados;
                            $pendientes[] = $pendiente;
                        }
                        // Actualizamos la línea con la cantidad confirmada
                        if($confirmados > 0) {
                            $l->cantidad = $confirmados;
            ...
            if ($continuar) {
                ...
                // Actualizamos los subtotales del pedido y albaran, ya que se han eliminado artículos
                if($modificado) {
                    $this->pedido->neto = 0;
                    $this->pedido->totaliva = 0;
                    $this->pedido->totalirpf = 0;
