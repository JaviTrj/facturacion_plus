...
class ventas_articulo extends fbase_controller
{
    ...
    protected function private_core()
    {
        ...
        if ($this->articulo) {
            ...
            // Mostrar los artículos de familia
            if($this->familia->codfamilia) {
                $atributo_familia = new atributo_familia();
                $this->atributos_familia = $atributo_familia->get_atributos_de_familia($this->familia->codfamilia,TRUE);
                $valor = new atributo_articulo_familia();
                $valores = $valor->get_atributos_de_articulo($this->articulo->referencia);
                foreach($this->atributos_familia as &$atributo) {
                    $valor = array_find($valores, function($v) use($atributo) {
                        return $v->idatributofamilia == $atributo->idatributofamilia;
                    });
                    $atributo->valor = is_null($valor) ? "" : $valor->valor;
                }
            }
        } else {
    ...
    private function modificar_articulo()
    {
        ...
        $oldfamilia = $this->articulo->codfamilia;
        ...
        $oldnostock = $this->articulo->nostock;
        ...
        // Primero comprobamos que se están añadiendo todos los atributos
        $atributo = new atributo_familia();
        $atributos = $atributo->get_atributos_de_familia($this->articulo->codfamilia);
        $checked_atributos = array();
        foreach($atributos as $atr) {
            if(!isset($_POST["atrfam_" . $atr->idatributofamilia])) {
                $this->new_error_msg("El atributo ". $atr->nombre." no está definido para el artículo");
                return;
            }
            else {
                $valor = $_POST["atrfam_" . $atr->idatributofamilia];
                $checked_atributos[] = new atributo_articulo_familia(array("referencia"=>$this->articulo->referencia,"idatributofamilia"=>$atr->idatributofamilia,"valor"=>$valor));
            }
        }
        
        // Después eliminamos los antiguos si hubo cambio de familia
        if($oldfamilia != $this->articulo->codfamilia) {
            $dato = new atributo_articulo_familia();
            $atributos_viejos = $atributo->get_atributos_de_familia($oldfamilia);
            foreach($atributos_viejos as $atr) {
                $dato->delete_atributo_from_articulos($atr->idatributofamilia,array($this->articulo->referencia));
            }
        }

        // Y por último añadimos los nuevos
        foreach($checked_atributos as $dato) {
            if(strlen($dato->valor)) {
                $dato->save();
            }
            else {
                $dato->delete();
        ...
        if ($this->articulo->save()) {
            ...
            // Si se deja de controlar stock, eliminar todas las reservas en pedidos
            if($this->articulo->nostock && !$oldnostock) {
                $linea = new linea_pedido_cliente();
                $lineas = $linea->all_from_articulo($this->articulo->referencia);
