...
class nueva_compra extends fbase_controller
{
    ...
    protected function private_core()
    {
        ...
        if (isset($_REQUEST['buscar_proveedor'])) {
            ...
            $this->new_search();
        } else if (isset($_GET['generar_pedido'])) {
            $this->generar_con_pendientes_de_comprar();
    ...
    private function new_search()
    {
        ...
        $this->results = $articulo->search_con_atributos($this->query, 0, $_REQUEST['codfamilia'], isset($_REQUEST['con_stock']), $_REQUEST['codfabricante'], FALSE, isset($_REQUEST['solo_proveedor']) ? $_REQUEST['codproveedor'] : '', crear_objeto_filtraje_por_atributos_familia());
    ...
    private function nuevo_pedido_proveedor()
    {
        ...
        if ($continuar) {
            ...
            if ($pedido->save()) {
                ...
                for ($i = 0; $i < $n; $i++) {
                    if (isset($_POST['referencia_' . $i])) {
                        ...
                        if ($linea->save()) {
                            ...
                            // Eliminar los pendientes si ha almacenado cuantos fueron, indicando que se tienen que pisar
                            if (isset($_POST['pisar_' . $i])) {
                                $a_pisar[] = array(
                                    'cuantos'=> +$_POST['pisar_' . $i],
                ...
                // Si el método de generar pendientes es sustituir los anteriores, en el pedido autogenerado (se sabe ya que se han puesto las cantidades a pisar)
                // se contaron los pendientes del proveedor, con los que al aceptar el pedido se tienen que eliminar
                if($continuar && count($a_pisar) > 0) {
                    $pendiente = new pendiente_proveedor();
                    if(!$pendiente->delete_articulos_de_proveedor($pedido->codproveedor,isset($_POST['todos_almacenes']) ? FALSE : $pedido->codalmacen, $a_pisar, true)) {
                        $this->new_error_msg("No fue posible eliminar los pendientes que se están pidiendo de nuevo, con lo que se elimina el pedido");
                        $continuar = FALSE;
                    }
                }
            ...
            if ($albaran->save()) {
                ...
                for ($i = 0; $i < $n; $i++) {
                    if (isset($_POST['referencia_' . $i])) {
                        ...
                        if ($linea->save()) {
                            if ($articulo) {
                                // Si es un albarán de pendientes, eliminarlos de pendientes
                                if(isset($_POST['pendientes_proveedor'])) {
                                    $pendiente = new pendiente_proveedor();
                                    $art = array("referencia"=>$articulo->referencia, "cuantos"=>$linea->cantidad);
                                    if(!$pendiente->delete_articulos_de_proveedor($albaran->codproveedor, $albaran->codalmacen, array($art), false)) {
                                        $this->new_error_msg("No fue posible eliminar los pendientes que se están pidiendo de nuevo, con lo que se elimina el pedido");
                                        $continuar = FALSE;
                                    }
                                }
    ...
    private function generar_con_pendientes_de_comprar()
    {
        $articulo_caller = new articulo();
        $almacen = isset($_GET["codalmacen"]) ? $_GET["codalmacen"] : FALSE;
        $proveedor = $_GET["generar_pedido"];
        $divisa = isset($_GET['coddivisa']) && $_GET['coddivisa'] != $this->empresa->coddivisa ? $_GET['coddivisa'] : FALSE;
        $metodo = new proveedor_opcion_pendientes();
        $metodo = $metodo->get($proveedor);
        $articulos = $articulo_caller->pendientes_de_proveedor($proveedor,$almacen,$metodo->codopcion == 'si_pisar');
        $results = array();
        ...
        if($articulos) {
            ...
            foreach($articulos as $articulo) {
                ...
                $ap = $this->articulo_prov->get_by($articulo->referencia, $proveedor);
                if ($ap) {
                    $coste = $ap->precio;
                    $dto = $ap->dto;
                }
                ...
                /// convertimos la divisa
                if ($divisa) {
                    $coste = $this->divisa_convert($coste, $this->empresa->coddivisa, $divisa);
                }
                ...
                $result = array(
                    ...
                    "pendientes"=>$articulo->pendientes
                );
                if(isset($articulo->pendientes_proveedor)) {
                    $result["pisar"] = $articulo->pendientes_proveedor;
                }
                $results[] = $result;
            }
        }
